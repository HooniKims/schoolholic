...
- [x] 알림장 AI `Generation` 라벨 및 반복 메타 출력 재보정
  - [x] `Generation.`, `생성:`, `한국어 다듬기`, `최종 검토`, `한국어 본문 + 이모지` 등 모델이 새로 붙인 라벨 제거
  - [x] 깨진 이모지 대체 문자(`��`)는 기본 플랫 이모지로 복구하고 정상 이모지는 유지
  - [x] 이중 `---`가 출력될 경우 마지막 구분선을 기준으로 최종 한국어/영어 본문만 추출
  - [x] 원문 숫자/날짜/시간이 결과에서 빠지면 자동 재시도하고, 계속 누락되면 오류 처리
  - [x] 실제 LLM 호출로 라벨 제거, 구분선 유지, 깨진 문자 제거, 원문 핵심 숫자 보존 확인
- [x] 알림장 AI 다듬기/영어 번역 항목 누락 및 메타 출력 재발 방지 - 2026-05-12
  - [x] 원인 확인: 영어 번역 옵션에서 한국어/영어 결합 결과를 다시 후처리할 때 마지막 구분선과 뒤쪽 이모지 블록만 최종 본문으로 오인하여 첫 항목이 잘릴 수 있었음
  - [x] `lib/notice-content.ts` 후처리 강화: 프롬프트 제한사항, 작업 설명, 이모지 선택 설명, 영어 번역 지시문이 본문에 섞여도 제거하고 최종 알림장 본문만 남기도록 보강
  - [x] `lib/notice-ai.ts` 생성 안정화: 원문 항목 수와 최종 줄 수 일치 검사, 누락/중복 시 재시도, 영어 번역 줄 누락 시 재번역, 실패 시 안전한 영어 대체문 생성 적용
  - [x] 번호 목록 원문도 최종 결과에서는 가벼운 이모지 항목으로 정리되도록 보강하고, 이모지가 빠진 한국어 줄은 내용에 맞는 이모지로 보정
  - [x] 회귀 테스트 보강: 첫 항목 누락, 영어 번역 첫 줄 누락, 중복 출력, 메타/프롬프트 제한사항 출력, 항목 수 불일치 케이스 추가
  - [x] 실제 로컬 모델 반복 검증: `gemma4:e4b`, `gemma4:e2b`, `lmstudio:gemma-4-26b-a4b-it-q4ks` 각각 10회씩 총 30회 실행, 실패 0건 확인
  - [x] `npm test`, `npm run lint`, `npx tsc --noEmit`, `npm run build` 검증 완료
  - [x] GitHub 업로드 전 임시 반복검증 스크립트 및 로그성 파일은 저장소에 남기지 않도록 정리
